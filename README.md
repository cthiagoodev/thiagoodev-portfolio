# 🚀 Portfólio de Projetos - Foco em Go e Infraestrutura Self-Hosted

Este projeto atua como meu **laboratório prático** e **vitrine profissional**, com o objetivo principal de consolidar o conhecimento em **Desenvolvimento Backend com Go** e **Infraestrutura Própria (Self-Hosted)**.
...
* **Integração:** Utiliza a **API Pública do GitHub** consumida nativamente ou via clientes HTTP em Go.
* **Scheduler Inteligente:** Uma rotina nativa em Go (utilizando goroutines e tickers) executa periodicamente a busca por atualizações.
...
| :--- | :--- | :--- |
| **Backend** | **Go (Golang)** | Utilização de recursos modernos e eficientes da linguagem para alta performance, concorrência nativa e baixo consumo de memória. |
| **Roteador** | **Chi v5** | Roteador leve, rápido e idiomático para a construção de serviços RESTful e gerenciamento de middlewares. |
...
| :--- | :--- | :--- |
| **Banco de Dados** | **PostgreSQL** | Conexão otimizada via **pgx/v5** e rodando em container Docker com **volumes persistentes** para durabilidade dos dados. |
...
| **Containerização** | **Docker & Compose** | Empacota a aplicação Go (multi-stage build para gerar um binário minimalista) e serviços de suporte para isolamento total. |
